...
namespace Backend\Modules\Spotlight\Engine;
...
class Model
{
    ...
    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         ...
         LEFT OUTER JOIN spotlight AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    {
        BackendModel::get('database')->delete('spotlight', 'id = ?', (int) $id);
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('spotlight_categories', 'id = ?', array((int) $id));
            $db->update('spotlight', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM spotlight_categories AS i
                 LEFT OUTER JOIN spotlight AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('spotlight', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('spotlight_categories', $item);
    }
