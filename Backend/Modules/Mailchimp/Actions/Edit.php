...
class Edit extends ActionEdit
{
    ...
    protected function loadData()
    {
        ...
        if ($this->id == null || !BackendMailchimpModel::exists($this->id)) {
            $this->redirect(
                Model::createURLForAction('Index') . '&error=non-existing'
            );
        }

        $this->record = BackendMailchimpModel::get($this->id);
    }
    ...
    protected function loadForm()
    {
        ...
        // get categories
        $categories = BackendMailchimpModel::getCategories();
        $this->frm->addDropdown('category_id', $categories, $this->record['category_id']);

        // meta
        $this->meta = new Meta($this->frm, $this->record['meta_id'], 'titel', true);
        $this->meta->setUrlCallBack('Backend\Modules\Mailchimp\Engine\Model', 'getUrl', array($this->record['id']));
    ...
    protected function validateForm()
    {
        if ($this->frm->isSubmitted()) {
            ...
            if ($this->frm->isCorrect()) {
                ...
                BackendMailchimpModel::update($item);
                $item['id'] = $this->id;
