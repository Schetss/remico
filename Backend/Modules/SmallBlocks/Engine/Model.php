...
namespace Backend\Modules\SmallBlocks\Engine;

use Backend\Core\Engine\Model as BackendModel;
use Backend\Core\Engine\Language;

/**
 * In this file we store all generic functions that we will be using in the Small blocks module
 *
...
class Model
{
    const QRY_DATAGRID_BROWSE =
        'SELECT i.id, i.title, UNIX_TIMESTAMP(i.created_on) AS created_on, i.sequence
         FROM small_blocks AS i
         WHERE i.language = ?
         ORDER BY i.sequence';

    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         FROM small_blocks_categories AS c
         LEFT OUTER JOIN small_blocks AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    {
        BackendModel::get('database')->delete('small_blocks', 'id = ?', (int) $id);
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('small_blocks_categories', 'id = ?', array((int) $id));
            $db->update('small_blocks', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
    ...
    public static function existsCategory($id)
    {
        return (bool) BackendModel::get('database')->getVar(
            'SELECT 1
             FROM small_blocks_categories AS i
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM small_blocks_categories AS i
                 LEFT OUTER JOIN small_blocks AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    /**
     * Get the maximum Small blocks sequence.
     *
     * @return int
     */
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('small_blocks', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('small_blocks_categories', $item);
    }
